...
int executeSimpleOperation(machine_state_t *state, y86_instruction_t *instr)
{
    ...
    case I_RMMOVQ:
        state->programCounter = instr->valP;
        success = memWriteQuadLE(state, instr->valC + state->registerFile[instr->rB], state->registerFile[instr->rA]);
        break;
    case I_MRMOVQ:
        state->programCounter = instr->valP;
        success = memReadQuadLE(state, instr->valC + state->registerFile[instr->rB], &valRead);
        state->registerFile[instr->rA] = valRead;
        break;
    default:
        success = 0;
    }
